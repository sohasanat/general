        ...
        <div class="relative py-3 sm:max-w-sm sm:mx-auto">
            ...
            <div class="relative px-4 py-10 bg-gradient-to-t from-orange-100 to-[#f5f1e4] sm:rounded-3xl sm:p-14">
                ...
                <div class="max-w-md mx-auto">
                    <div>
                        <h1 class="text-xl text-center font-YekanBakh-Bold">تغییر رمز</h1>
                    </div>
                    <form action="{{ route('passwordchange') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="divide-y divide-gray-200">
                            <div class="py-8 text-base leading-6 space-y-4 text-gray-700 sm:text-lg sm:leading-7">
                                <input name="user" class="hidden" value="{{$passcode}}">

                                <input name="pass1" type="password" placeholder="رمز عبور:" class="input w-full rounded-full focus:outline-none placeholder:text-sm" />
                                <input name="pass2" type="password" placeholder="تکرار رمز عبور:" class="input w-full rounded-full focus:outline-none placeholder:text-sm" />
                                <button class="btn bg-stone-800 w-full hover:bg-stone-900 text-white rounded-full text-sm" type="submit">ثبت رمز</button>
                            </div>
                        </div>
                    </form>
                </div>
